...
;     v0.2 2009Mar22    J.D. Monnier    ra,dec,equinox
;
...
if (keyword_set(status) eq 1) then begin
   ...
   t3_new(*).ra         = oitarget(tin).raep0
   t3_new(*).dec        = oitarget(tin).decep0
   t3_new(*).equinox    = oitarget(tin).equinox
