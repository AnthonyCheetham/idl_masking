...
;  rat995_240_320 = m[s[rnum-1]]/0.67 ;*0.67 due to the super-gaussian window
pro binary_grid_edit,  infile,  printfile = printfile, no_bfinfo=no_bfinfo, usevis = usevis_in,  nfr = nfr,  nosim = nosim,  $
 init_crat = init_crat,  maxsep = maxsep,  fix_crat = fix_crat,  cp_cov =  cp_cov_in,  minsep =minsep,  $
 ...
 nsim=nsim, tom_this_doenst_work_for_nirc2=tom_this_doenst_work_for_nirc2, t3errscale=t3errscale,plot_model=plot_model
...
if (cp_cinv[0] eq -1) then begin
 ...
 cperr_null = sqrt(t3data.t3phierr^2+extra_error_null^2)*sqrt(n_elements(good)/ndf)
 if (keyword_set(scale_err)) then begin
     t3data.t3phierr = t3data.t3phierr*sqrt(newchi2[0]*n_elements(good)/ndf) 
 endif else t3data.t3phierr = sqrt(t3data.t3phierr^2 + extra_error^2)*sqrt(n_elements(good)/ndf)
...
chi2_bin =  binary_oifits_chi2(bestp, sig = perror,  corr = cor,  covar = covar, /print)
...
if (p ne '') then printf,  1,  chi2_null, format = '("Chi-squared (null):   ",F7.2)' 
print, sqrt(chi2_null-chi2_bin),          format = '("Significance:         ",F7.2,"  = sqrt(chi2_null - chi2_bin)")'
if (p ne '') then printf,  1,  sqrt(chi2_null-chi2_bin), format = '("Significance:        ",F7.2,"  = sqrt(chi2_null - chi2_bin)")'
...
;;ACC: Do a nice plot of model vs data for the best fit:
if keyword_set(plot_model) then begin
    window,1
    best_modelt3=binary_t3data([bestp,0.1,0.1],t3data=t3data)
    plotrange=[min([best_modelt3.t3phi,t3data.t3phi]),max([best_modelt3.t3phi,t3data.t3phi])]
    plot,t3data.t3phi,modelt3.t3phi,psym=5,xtitle='Measured Closure Phase (deg)',ytitle='Model Closure Phase (deg)',charsize=1.5,yr=plotrange,xr=plotrange
    oplot,[-360,360],[-360,360],color=100
    oplot,[0,0],[-360,360],color=200
    wset,0
endif
...
if (keyword_set(nosim) eq 0) then begin
 ...
 if (cp_cinv_null[0] eq -1) then dm =  diag_matrix(1./cperr_null^2) else dm = cp_cinv_null
 if (proj[0] ne -1) then begin
  for i = long(0), long(nr)*nang-1 do norm[i] = (modcp[*,i]#proj)^2#(1./proj_err^2)
 ;;*** Are you MJI? If not you only care about the following line (2) *** 
 endif else for i = long(0), long(nr)*nang-1 do norm[i] = modcp[*, i]#dm#modcp[*, i]
...
;;     new_crats =  regrid_fft(crats, 2*nr, 2*nang, /xrev)
 ...
 annuli = [10,20,40,80,160,240,320]
 ...
 print, confidence_level*100.0,  format = '(F4.1,"% limits:      MJD     10-20   20-40   40-80  80-160 160-240 240-320")'
 print, target_name,t3data[0].mjd,  -2.5*alog10(rat_annuli),    format = '(A15, "& ", F7.1, 6(" & ", F5.2))'
 print, t3data[0].mjd,  -2.5*alog10(rat99_annuli),  format = '("99% only       & ", F7.1, 6(" & ", F5.2))'
 if (p ne '') then begin
  printf,1, 'mrg.oifits file used: ',infile
  printf,1, confidence_level*100.0,  format = '(F4.1,"% limits:      MJD     10-20   20-40   40-80  80-160 160-240 240-320")'
  printf,1, target_name, t3data[0].mjd, -2.5*alog10(rat_annuli), format = '(A15, "& ", F7.1, 6(" & ", F5.2), "\\ LIMITS")'
  printf,1, t3data[0].mjd, -2.5*alog10(rat99_annuli), format = '("99% only       & ", F7.1, 6(" & ", F5.2), "\\ L99")'
 endif  
