...
pro binary_grid_sylv,  infile,  printfile = printfile, no_bfinfo=no_bfinfo, usevis = usevis_in,  nfr = nfr,  nosim = nosim,  $
 ...
 nsim=nsim, tom_this_doenst_work_for_nirc2=tom_this_doenst_work_for_nirc2, t3errscale=t3errscale
...
;;ACC HACK!
w=where(t3data.target eq 'T_Cha',complement=cals)
t3data=t3data[w]

if (keyword_set(t3errscale)) then t3data.t3phierr *= t3errscale
w = where(t3data.flag)
;if (w[0] ne -1) then t3data[w].t3phierr = 1e4 ;;ACC HACK! FOR SYLVESTRES OIFITS FILES!
...
;;ACC HACK!
w2=where(vis2data.target eq 'T_Cha',complement=cals)
vis2data=vis2data[w2]
