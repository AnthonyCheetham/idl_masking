...
if option eq 1 then begin
    ...
    while (1) do begin
        ...
        image_cont,plot_im,/n,tit='Current cal4src',xval=xvals,yval=yvals,xtit='Calibrators',ytit='Targets'
        ...
        if wx[0] eq -1 or wy[0] eq -1 then begin
            if not keyword_set(unlock_all) then print,'You missed! Try again!'
        ...
        if (keyword_set(unlock_all) and (wx[0] eq -1 or wy[0] eq -1) )then begin
            ...
            if wx[0] eq -1 and wy[0] eq -1 then begin ;; cal-targ
               wx=where(blocks2[1,*] ge x and blocks2[0,*] le x)
               wy=where(blocks[1,*] ge y and blocks[0,*] le y)
               ;;take the indices from the calibrator blocks
               cix1=blocks2[0,wx[0]]
               cix2=blocks2[1,wx[0]]
               six1=blocks[0,wy[0]]
               six2=blocks[1,wy[0]]
            endif else if wx[0] eq -1 then begin ;; then you want targ-targ
               wx=where(blocks2[1,*] ge x and blocks2[0,*] le x)
               ;;take the indices from the calibrator blocks
               cix1=blocks2[0,wx[0]]
               cix2=blocks2[1,wx[0]]
               six1=blocks2[0,wy[0]]
               six2=blocks2[1,wy[0]]
            endif else if wy[0] eq -1 then begin ;; then you want cal-cal
               wy=where(blocks[1,*] ge y and blocks[0,*] le y)
               cix1=blocks[0,wx[0]]
               cix2=blocks[1,wx[0]]
               six1=blocks[0,wy[0]]
...
image_cont,new,/n,tit='Final cal4src Matrix',xval=xvals,yval=yvals,xtit='Calibrators',ytit='Targets'
;xyouts,blocks[0,*],replicate(yoffset,ncals),cal_label,/data,charsize=2,charthick=2
;xyouts,replicate(xoffset,nsrc),((blocks2[0,*]+blocks2[1,*])/2.),src_label,/data,charsize=2,charthick=2
