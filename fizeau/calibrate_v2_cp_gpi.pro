...
;       save_python - Also save a copy of the oifits file in a python friendly format
;                     (unfortunately we use a dodgy hack in binary grid that breaks the
;                      oifits standard, so the python oifits package cant open them)
;     save_all_clps - calibrates all raw closure phases rather than just the mean, and saves them all into oifits. This is useful when the signal to noise is high enough that the rotation of the instrument needs to be taken into account


pro calibrate_v2_cp_gpi, cubeinfo_file,  root_dir=root_dir, reset = reset, apply_phscorr = apply_phscorr, $
                     ...
                     freeze=freeze, disp_fit_file=disp_fit_file,save_cov=save_cov,save_python=save_python,$
                     no_cp_rejection=no_cp_rejection,save_all_clps=save_all_clps
...
if keyword_set(save_all_clps) then i_want_a_memory_leak=1
...
if keyword_set(cal4src) then begin
    ...
    if cal4srcsz[0] eq 2 then clog.cal4src=cal4src else begin
        ...
        if cal4src[0] gt 0  then clog.cal4src=mega_cal4src(dir+cubeinfo_file)
...
;; Let's work out the wavelengths now in case we need them later
;; u_ideal is the baseline divided by lambda so we can get them back from the first baseline like this
wavs = reform((xy_coords[bl2h_ix[0,0],0] - xy_coords[bl2h_ix[1,0],0])/u_ideal[0,*] )
...
used_quantities =  {v2:fltarr(n_baselines,n_wavs), v2corr:fltarr(n_baselines,n_wavs),  $
                    v2_err:fltarr(n_baselines,n_wavs), cp:fltarr(n_bispect,n_wavs),  cp_err:fltarr(n_bispect,n_wavs),$
                    cp_cov:fltarr(n_bispect, n_bispect,n_wavs), $
                    bs_all: Ptr_New( /Allocate_Heap ), amp:fltarr(n_baselines,n_wavs), $
                    amp_err:fltarr(n_baselines,n_wavs),  phase:fltarr(n_baselines,n_wavs), $
                    phase_err:fltarr(n_baselines,n_wavs), flag:intarr(n_baselines,n_wavs), $
                    bsflag:intarr(n_bispect,n_wavs), bs_amp:fltarr(n_bispect,n_wavs),bs_amp_err:fltarr(n_bispect,n_wavs)}
...
for i = 0, ncubes-1 do begin
    ...
    all_quan[i]=  get_used_quantities_gpi(filestring,  bad_baselines,  bad_bs, root_dir,$
    ...
    ;; For IFU data, mark a hole as bad if its V2 is low in more than
    ;; 1/3 of the wavelength channels.
    If( NOT skip_baseline_rejection ) THEN BEGIN
        if( (total(clog.cal4src, 2) )[i] gt 0 || src_for_bad_holes) then begin
           bad_hole_count=intarr(n_holes)
           yfits=dblarr(n_baselines,n_wavs)
           ;;precompute the estimated V2 vs wavelength
           for wav=0,n_wavs-1 do begin
              dummy =linfit(sqrt(u[*,wav]^2+v[*,wav]^2), alog(all_quan[i].v2[*,wav] > 0.001),  yfit = yfit)
              yfits[*,wav]=yfit
              ;;loop through holes and mark the bad ones
              for j = 0, n_holes -1 do begin
                 w =  where(bl2h_ix[0, *] eq j or bl2h_ix[1, *] eq j)
                 if (mean(all_quan[i].v2[w,wav]/exp(yfits[w,wav])) lt 0.3) then bad_hole_count[j]+=1
              endfor
           endfor

           ;;now go through the bad holes
           bad_hs=where(bad_hole_count gt n_wavs/3.)
           if max(bad_hs) ne -1 then begin
              for bad_hole_ix=0,n_elements(bad_hs)-1 do begin
                 j=bad_hs[bad_hole_ix]
                 w =  where(bl2h_ix[0, *] eq j or bl2h_ix[1, *] eq j)
                 plot, r, all_quan[i].v2/exp(yfits), psym=4,  xtitle = 'Baseline (wavelengths)',$
                       ytitle =  'V^2 / Fitted V^2',  xr = [0, max(r)]
                 oplot,  r[w,*], all_quan[i].v2[w,*]/exp(yfits[w,*]), psym=2
   ...
   if (n_elements(w) gt 6 and not keyword_set(see_all)) then begin
      ...
      for j=0,n_elements(w)-1 do begin
         this_cp_arr = all_quan[src[i]].cp[good_bispect,*] - all_quan[w[j]].cp[good_bispect,*] $
                       + randomn(seed,[n_elements(good_bispect),n_wavs])*1e-6
      ...
      if (not keyword_set(cal_cp_inspect)) then begin
         ...
         ; print,'ACC hack to skip ignoring calibrators for SPHERE'
      endif else begin
         m = median(cp_rms)*(180.0/!pi)^2 ;
          plothist,cp_rms*(180.0/!pi)^2,bin=(1.5*m)/10.0,xhist,yhist,/noplot
         plot,xhist,yhist,xr=[0,3*m],psym=10,$
         ...
         al_legend,['Median'],line=[1],/right_leg,charsize=2
      ...
      for j = 0, n_elements(w)-1 do begin
         plothist,  180/!pi*(all_quan[src[i]].cp[good_bispect] - $
                        ...
                        xhist,yhist,bin = median(abs(all_quan.cp[good_bispect]))/4*180/!pi,/noplot
         plot,xhist,yhist,psym=10, charsize=charsize, xtitle = 'Closure phase (degs)'
      endfor
      If( keyword_set( display_calibs_too ) EQ 1 ) THEN BEGIN
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ...
   if (ncal ge 1) then for wav=0,n_wavs-1 do begin
      for j =  0, n_baselines -1 do begin
         cmn =  wtmn(all_quan[w].v2[j,wav], all_quan[w].v2_err[j,wav], sdev)
         calib_src_quan[i].v2[j,wav] =  all_quan[src[i]].v2[j,wav]/cmn
         calib_src_quan[i].v2_err[j,wav] =  sqrt(all_quan[src[i]].v2_err[j,wav]^2/cmn^2 + $
                                             sdev^2*all_quan[src[i]].v2[j,wav]^2/cmn^4)
         cmn =  wtmn(all_quan[w].amp[j,wav], all_quan[w].amp_err[j,wav], sdev)
         calib_src_quan[i].amp[j,wav] =  all_quan[src[i]].amp[j,wav]/cmn
         calib_src_quan[i].amp_err[j,wav] =  sqrt(all_quan[src[i]].amp_err[j,wav]^2/cmn^2 + $
                                              sdev^2*all_quan[src[i]].amp[j,wav]^2/cmn^4)  
      endfor
   ...
   if (ncal ge 1) then for wav=0,n_wavs-1 do begin
      ...
      if keyword_set(save_all_clps) then begin
         ;;also calibrate bs_all
         bs_all=*all_quan[src[i]].bs_all
         nf=(size(bs_all))[1]
      endif
         
      for j =  0, n_bispect -1 do begin
         cmn =  wtmn(all_quan[w].bs_amp[j,wav], all_quan[w].bs_amp_err[j,wav], sdev)
         calib_src_quan[i].bs_amp[j,wav] =  all_quan[src[i]].bs_amp[j,wav]/cmn
         calib_src_quan[i].bs_amp_err[j,wav] =  sqrt(all_quan[src[i]].bs_amp_err[j,wav]^2/cmn^2 + sdev^2*all_quan[src[i]].bs_amp[j,wav]^2/cmn^4)
         if keyword_set(save_all_clps) then bs_all[*,j,wav]/=cmn
      endfor ;;other case handled above

      if keyword_set(save_all_clps) then *calib_src_quan[i].bs_all=bs_all
   endfor
   ...
   for wav=0,n_wavs-1 do begin
      ...
      if (ncal ge 1) then for j = 0, n_bispect-1 do begin
         cmn[j]  =  wtmn(all_quan[w].cp[j,wav], all_quan[w].cp_err[j,wav], sdev,  wts = wts)
         cerr[j] = sdev
         cwts[j, *] = wts
      endfor
      ...
      if (w[0] ne -1) then for j = 0, n_elements(w)-1 do $
         ccp_cov += ( cwts[*, j] # transpose(cwts[*, j]) ) * all_quan[w[j]].cp_cov[*,*,wav]
      if (clog.subtract_cp eq 1) then begin
         calib_src_quan[i].cp[*,wav] =  all_quan[src[i]].cp[*,wav] - cmn 
         calib_src_quan[i].cp_err[*,wav] =  sqrt( all_quan[src[i]].cp_err[*,wav]^2 + cerr^2 )
         calib_src_quan[i].cp_cov[*,*,wav] = all_quan[src[i]].cp_cov[*,*,wav] + ccp_cov

         if keyword_set(save_all_clps) then begin
            ;;update only this wavelength
            bs_cal=exp(complex(0,rebin(reform(cmn,[1,n_bispect]),[nf,n_bispect])))
            bs_all[*,*,wav]/=bs_cal
            *calib_src_quan[i].bs_all=bs_all
         endif

         ;;Multiple calibrator covariace matrices are now used correctly in the following
         ;;line... I think.
         ;;ACC: this next line probably needs editing somehow, but I can't work out how it works.. TODO
         cp2phase,  mf_filestring, calib_src_quan[i].cp[*,wav], calib_src_quan[i].cp_err[*,wav], $
                    all_quan[src[i]].cp_cov[*,*,wav]+ccp_cov, $
                    phase=phase, ph_err=ph_err,  diagonal_cov = clog.diagonal_cov,wav=wav
         calib_src_quan[i].phase[*,wav] =  phase
         calib_src_quan[i].phase_err[*,wav] =  ph_err
      endif else begin
         calib_src_quan[i].cp[*,wav] = all_quan[src[i]].cp[*,wav]
         calib_src_quan[i].cp_err[*,wav] = all_quan[src[i]].cp_err[*,wav]
         cp2phase,  mf_filestring, calib_src_quan[i].cp[*,wav], calib_src_quan[i].cp_err[*,wav], $
                    all_quan[src[i]].cp_cov[*,*,wav], $
...
u_save =  fltarr(n_baselines, nsrc,n_wavs)
v_save =  fltarr(n_baselines, nsrc,n_wavs)
savestrings = strarr(nsrc)

;;Save these initially bad baselines and bispect
...
;Allow for user-selected bad baselines...
        ...
        for wav=0,n_wavs-1 do good_baselines =  good_baselines[where(aquan.v2[good_baselines,wav]-aquan.v2_err[good_baselines,wav] gt 0)]
        ...
        ;;ACC: this display is pretty useless with this many points...
...
;Allow for user-selected closure-phase histogram cutoff.
       ...
       al_legend,['suggested lower cutoff'],line=[1],/right_leg

       print,  'Click on new error histogram lower cutoff (left of axis to accept)...'
       wait,  0.2
       if (not skip_cp_rejection) then cursor,  xL,  y else begin
          xL=low_cutoff
          if keyword_set(no_cp_rejection) then xL=1e-9 ;; a small, nonzero number
       endelse
       if (xL gt 0) then begin
          plothist,  aquan.cp_err[good_bispect,*]*180/!pi,  xr = [0, 8*m],  bin = m/10,  xtit = 'CP error (degs)'
        oplot,  [high_cutoff, high_cutoff],  [0, 1000],  line = 1
        oplot,  [xL,xL], [0, 1000], line=2
        al_legend,['suggested upper cutoff','selected lower cutoff'],line=[1,2],/right_leg
        print,  'Click on cutoff for bad closure-phases (left of axis to accept)...'
        wait,  0.2
        
        low_cutoff=xL ;; ACC: this line was missing...
        ...
        if (xU gt 0) then begin
           ...
           al_legend,['suggested upper cutoff','selected lower cutoff','selected upper cutoff'],$
...
;Next the wavelength - only use one per save file.
    define_oiwavelength, oiwavelength_unit, nwave = n_wavs
    oiwavelength = oiwavelength_unit
    oiwavelength.extver = 1
    oiwavelength.insname = olog.instrument[src[asrc]]
    ; eff_wave=filter[0]+filter[1]*(findgen(n_wavs)/n_wavs - 0.5)
    ; eff_wave = filter[0]+filter[1]*(findgen(n_wavs)/(n_wavs-1) - 0.5) ;;ACC edit Aug 2014
    eff_wave = wavs ;;ACC edit Aug 2018
    eff_band = replicate((max(wavs)-min(wavs))/(n_wavs-1),n_wavs) ;;assuming equally spaced bands
...
;Next the v^2 information
    ...
    define_oivis2, oivis2_unit, nwave = n_wavs
    ...
    oivis2[*].ucoord    = u1[g,0]*eff_wave[0]
    oivis2[*].vcoord    = v1[g,0]*eff_wave[0]
    oivis2[*].sta_index  = bl2h_ix[*, g]

;Last, the closure phase information
    define_oit3, oit3_unit, nwave = n_wavs
    if keyword_set(save_all_clps) eq 0 then begin
       ;; the old way: collapse clps down to one block.
       oit3 = replicate(oit3_unit, n_bispect)
       for i =  0,  n_bispect-1 do begin
          oit3[i].t3amp    = ptr_new(reform(aquan.bs_amp[i,*]))     ; ptr_new(0.0)
          oit3[i].t3amperr = ptr_new(reform(aquan.bs_amp_err[i,*])) ;ptr_new(1.0)
          oit3[i].t3phi 	= ptr_new(reform(aquan.cp[i,*]*180.0/!pi))
          oit3[i].t3phierr = ptr_new(reform(aquan.cp_err[i,*]*180.0/!pi))
          oit3[i].flag 	= ptr_new(reform(aquan.bsflag[i,*]))
       endfor
       oit3[*].u1coord    = reform(u1[bs2bl_ix[0, *],0]*eff_wave[0])
       oit3[*].v1coord    = reform(v1[bs2bl_ix[0, *],0]*eff_wave[0])
       oit3[*].u2coord    = reform(u1[bs2bl_ix[1, *],0]*eff_wave[0])
       oit3[*].v2coord    = reform(v1[bs2bl_ix[1, *],0]*eff_wave[0])
       ...
       ;; save all clps with an error multiplied by sqrt(N) (i.e. stddev rather than SEM), and take rotation into account.
       oit3 = replicate(oit3_unit, n_bispect*nf) ;;nf is the number of frames
       for frame=0,nf-1 do begin
          for i =  0,  n_bispect-1 do begin
             ix=frame*n_bispect+i
             oit3[ix].t3amp    = ptr_new(reform(abs((*aquan.bs_all)[frame,i,*])))
             oit3[ix].t3amperr = ptr_new(reform(aquan.bs_amp_err[i,*]*sqrt(nf)))
             oit3[ix].t3phi    = ptr_new(reform(atan((*aquan.bs_all)[frame,i,*],/phase)*180.0/!pi))
             oit3[ix].t3phierr = ptr_new(reform(aquan.cp_err[i,*]*180.0/!pi *sqrt(nf)))
             oit3[ix].flag     = ptr_new(reform(aquan.bsflag[i,*]))
             oit3[ix].sta_index  = [bl2h_ix[0, bs2bl_ix[0, i]], $
                                  bl2h_ix[1, bs2bl_ix[0, i]],  $
                                  bl2h_ix[1, bs2bl_ix[1, i]]]
          endfor
          ix1=frame*n_bispect
          ix2=(frame+1)*n_bispect-1
          
          ;;Now correct the u and v coordinates for the parallactic angle.
          ;;If this next line fails then you arent saving all of the parralactic angles in the olog file!
          thepa = (*olog.pas[src[asrc]])[frame] ;;use the actual pa from this frame
          u1 =  olog.uflip*u*cos(thepa*!pi/180.0)  + v*sin(thepa*!pi/180.0)
          v1 = -olog.uflip*u*sin(thepa*!pi/180.0) +  v*cos(thepa*!pi/180.0)
          
          oit3[ix1:ix2].u1coord    = reform(u1[bs2bl_ix[0, *],0]*eff_wave[0])
          oit3[ix1:ix2].v1coord    = reform(v1[bs2bl_ix[0, *],0]*eff_wave[0])
          oit3[ix1:ix2].u2coord    = reform(u1[bs2bl_ix[1, *],0]*eff_wave[0])
          oit3[ix1:ix2].v2coord    = reform(v1[bs2bl_ix[1, *],0]*eff_wave[0])
       endfor
    endelse
...
define_oivis, oivis_unit, nwave= n_wavs
...
for i =  0, n_baselines-1 do begin
   ...
   if (known_dir[i] eq 1) then begin
      ...
      oivis[i].ucoord    = -u[bline_reorder[i]]*wavs[0]
      oivis[i].vcoord    = -v[bline_reorder[i]]*wavs[0]
      oivis[i].sta_index  = [bl2h_ix[1, bline_reorder[i]], bl2h_ix[0, bline_reorder[i]]]
   endif else begin
      oivis[i].visphi    = ptr_new(quan.phase[bline_reorder[i]]*180/!pi)
      oivis[i].ucoord     = u1[bline_reorder[i]]*wavs[0]
      oivis[i].vcoord     = v1[bline_reorder[i]]*wavs[0]
...
if keyword_set(save_cov) then begin
   ...
   writefits,savestrings[0]+'_python_mrg.oifits',cp_cov,hdr,/append
endif
