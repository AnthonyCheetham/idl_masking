;######## RELEASE CLIP HERE ######## 
function SXPAR_conica, hdr2, name, abort, COUNT=matches, COMMENT = comments, $
                                  NoContinue = NoContinue,nchar=nchar
;+
; NAME:
;      SXPAR
; PURPOSE:
...
;       NCHAR - ACC added this in case we need more than 8 chars from the header
;
...
;       P. Tuthill adapt for CONICA crazy long header keywords ... 
;-
;----------------------------------------------------------------------
...
if not keyword_set(nchar) then nchar=8
...
;       Check for valid header
  hdr=hdr2              ; prevent script changing header string
...
; PGT hack here. Strip out crazy long header keywords
; Concatenate final nchar chars into single keyword, strip out whitespace
 for h=0,s[1]-1 do begin
  ws=strpos(hdr[h],'=')  ; find equal sign
  if(ws gt nchar) then begin
    tempstr=strmid(hdr[h],0,ws)
    tempstr=strcompress(tempstr,/remove_all) ; remove whitespace
    sl=strlen(tempstr)
    tempstr=strmid(tempstr,sl-nchar,nchar)    ; strip out only final 8 chars
    hdr[h]=tempstr+strmid(hdr[h],ws)
  endif
 endfor
...
;  string.
   ...
   if strpos( nam, '*' ) EQ namelength1 then begin    
            ...
            num_length = nchar - name_length        ;Max length of number portion  
...
;  Otherwise, extend NAME with blanks to eight characters.
                ...
                while strlen(nam) LT nchar do nam = nam + ' ' ;Make 8 chars long
                vector = 0      
    endelse
...
;  value of the number field in NUMBER.
        ...
        if (not vector) and (start GE 0) then begin
            ...
            keyword = strmid(hdr[mn:mx], 0, nchar)
        endif else begin
            restart:
            start   = -1l
            keyword = strmid( hdr, 0, nchar)
        endelse
...
; Process string parameter 
 ...
 if matches GT 0 then begin
  line = hdr[nfound]
  svalue = strtrim( strmid(line,nchar+1,71),2)
  if histnam then $
        value = strtrim(strmid(line,nchar,71),2) else for i = 0,matches-1 do begin
                ...
                On_IOerror, GOT_VALUE
                  ...
                  or (strpos(value,'D') GE 0) then begin  ;Floating or double?
                      if ( strpos(value,'D') GT 0 ) or $  ;Double?
                         ( strlen(value) GE nchar ) then value = double(value) $
