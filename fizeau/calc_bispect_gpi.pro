...
;average_v2_not_power: instead of doing ( mean(splodge)/mean(flux) ), do mean(splodge/flux)

pro calc_bispect_gpi, cubeinfo_file,  root_dir=root_dir, ddir = ddir,  reset = reset,  plotall =plotall,  nosave = nosave, $
                  ...
                  subtract_bs_bias = subtract_bs_bias, window_mult=window_mult, special=special, cav=cav, rebin=rebin,$
                  disp_windowing=disp_windowing,average_v2_not_power=average_v2_not_power
...
; Begin analysis.
if (plotall ne 0) then set_plot, 'ps' else window,  0,  xsize = 1100,  ysize = 500
loadct,  39
for i=0, n_runs-1 do begin
    ...
    if (size(cube))[0] eq 4 then dimz = (size(cube))[4] else begin
       dimz=1
       cube=reform(cube,[dimx,dimy,n_frames,dimz])
    endelse
    ...
    if (keyword_set(cav)) then begin
        ...
        cube = rebin(cube, dimx, dimy, n_frames,dimz)
    endif
    if (keyword_set(add_noise)) then cube += add_noise*randomn(seed, dimx, dimy, n_frames,dimz)
    ...
    if(median_sub ne 0) then begin
        ...
        if median_sub eq 1 then for wav=0,dimz-1 do for k = 0, n_frames-1 do cube[*, *, k,wav] -= median((cube[*, *, k,wav])[w])
        if median_sub eq 2 then for wav=0,dimz-1 do for k = 0, n_frames-1 do cube[*, *, k,wav] -= mean((cube[*, *, k,wav])[w]) 
    endif
    ft_cube = complexarr(dimx,dimy,n_frames,dimz)
    if (olog.dk_fname[i] ne '') then begin
        dcube = readfits(ddir+olog.dk_fname[i])
        n_dframes =  (size(dcube))[3]
        if(median_sub eq 1) then for wav=0,dimz-1 do for k = 0,n_dframes-1 do dcube[*, *, k,wav] -=  median(dcube[*, *, k,wav])
        if(median_sub eq 2) then for wav=0,dimz-1 do for k = 0, n_dframes-1 do dcube[*, *, k,wav] -=  mean(dcube[*, *, k,wav]) 
        ft_dcube = complexarr(dimx,dimy,n_dframes,dimz)
    endif
    print, 'Doing Fourier Transforms for file: ',filename
    for wav=0,dimz-1 do begin
       for k = 0, n_frames-1 do begin
          ft_cube[*,*,k,wav] = fft(shift(cube[*,*,k,wav], -dimx/2,  -dimy/2)*window, 1)
          if keyword_set(disp_windowing) then begin ;;ACC: useful for checking windowing (e.g. make sure a wide companion is suppressed)
             !p.multi=[0,2,1]
             if dimz ne 1 then print,'  Up to wavelength:',wav
             image_cont,shift(shift(cube[*,*,k,wav], -dimx/2,  -dimy/2)*window,128,128),/n,/a,tit='Windowed'
             image_cont,shift(shift(cube[*,*,k,wav], -dimx/2,  -dimy/2),128,128),/n,/a,tit='No Window'
             wait,0.1
          endif
          if (showps eq 1) then begin
             temp =  modsq(ft_cube[0, 0, k,wav])/10.
             image_cont, shift(modsq(ft_cube[*,*,k,wav]) < temp,dimx/2,dimy/2)^0.5, /noc
             wait,  0.2
          endif
       endfor
    endfor
    if (olog.dk_fname[i] ne '') then for wav=0,dimz-1 do for k =  0, n_dframes -1 do $
       ft_dcube[*,*,k,wav] = fft(shift(dcube[*,*,k,wav], -dimx/2,  -dimy/2)*window, 1)
    ...
    bispect_gpi, ft_cube, mf_pvct, mf_gvct, mf_ix, mf_rmat, mf_imat, v2, v2_cov, bs,bs_var, bs_cov, bs_v2_cov, $
             ...
             average_v2_not_power=average_v2_not_power
    !p.multi = [0,3,1]
    ; !p.multi = [0,2,1]
    in = indgen(n_baselines)
    m = max(v2)
    ploterr, sqrt(u[*,0]^2+v[*,0]^2), v2[*,0], sqrt(v2_cov[in,in,0]), psym=5, yr = [0,min([m,1.0])],$
      ytitle = 'Raw V^2', xtitle = 'Baseline (wavelengths)',charsize=2

    for wav=1,dimz-1 do begin
       oploterr, sqrt(u[*,wav]^2+v[*,wav]^2), v2[*,wav], sqrt(v2_cov[in,in,wav]), psym=5 
    endfor

    ;;the June 2015 version of plothist doesnt plot the axes properly, so just use regular plot
    plothist, atan(bs, /phase)*180/!pi, xhist,yhist, xtitle = 'Closure Phase (degs)',/noplot
    plot,xhist,yhist,xtitle='Closure Phase (degs)',charsize=2,psym=10
    ...
    if (keyword_set(nosave) eq 0) then begin
       ...
       ;;ACC: calculate differential phases
       ; diff_ph_all=dblarr(n_frames,n_baselines,dimz-1)
       ; diff_ph=dblarr(n_baselines,dimz-1)
       ; diff_ph_sig=dblarr(n_baselines,dimz-1)
       ...
       ;; ACC put this hack here to test what is causing the naive calibration strategy in python from being so different to this pipeline
       ; print,'Hack in calc_bispect_gpi!'
       ; cp_all = atan(bs_all,/phase)
       ; cp = mean(cp_all,dim=1)
       ;;; end hack
...
; if (plotall eq 0) then window,  1,  xsize = 700,  ysize = 550, ypos=50
    ...
    mf_tot=fltarr(dimx,dimy,dimz)
    for wav=0,dimz-1 do begin
       dummy=fltarr(dimx,dimy)
       for j=0,n_baselines-1 do dummy[mf_pvct[mf_ix[0,j,wav]:mf_ix[1,j,wav]]]=mf_gvct[mf_ix[0,j,wav]:mf_ix[1,j,wav]]
       dummy+=rotate(shift(dummy,-1,-1),2)
       mf_tot[*,*,wav]+=dummy
    endfor
...
; image_cont, shift(alog((ps > max(ps)*1e-7)*(1-mask)),dimx/2,dimy/2), /nocont, /asp
                                ;loadct,  0
                                ;image_cont, shift(alog((ps[*,*,0] > max(ps[*,*,0])*1e-7)*(1-mask[*,*])),dimx/2,dimy/2), /nocont, /asp 
    for wav=0,dimz-1 do begin
       !p.multi=[1,3,1]
       image_cont, shift(alog((ps[*,*,wav] > max(ps[*,*,wav])*1e-7)*(1-mask[*,*,wav])),dimx/2,dimy/2), /nocont, /asp ,tit='Wavelength:'+strcompress(string(wav),/remove_all)
       wait,0.1
    endfor                        
    !p.multi = 0
...
if (plotall ne 0) then begin
    device,  /close
    spawn, '\mv idl.ps ' + print_prefix + strtrim(string(runs[i]),2) + print_ext
    set_plot,  'x'
endif
