...
pro bispect_gpi, ft_arr, mf_pvct, mf_gvct, mf_ix, mf_rmat, mf_imat, v2, v2_cov, bs,bs_var, bs_cov, bs_v2_cov, $
...
aveps=dblarr(dim1,dim2,dim3)
avedps=dblarr(dim1,dim2,dim3)   ;Do dark ps subtraction - for noise estimation...
...
for wav=0,dim3-1 do begin
   for i=0,n_baselines-1 do begin
      dummy=dblarr(dim1,dim2)
      pix=mf_pvct[mf_ix[0,i,wav]:mf_ix[1,i,wav]]
      dummy[pix]=1.0
      signal(*,*,wav) += dummy
   endfor
endfor
;; We need the mirror image as well, for the conjugate splodges
for wav=0,dim3-1 do signal[*,*,wav] = signal[*,*,wav] + shift(rotate(signal[*,*,wav],2),1,1)
;; and the central peak
d = dist(dim1)
central_peak=array_indices(d,where(d lt dim1/16))
for wav=0,dim3-1 do signal[central_peak[0,*],central_peak[1,*],fltarr(n_elements(central_peak)/2)+0]=1;; the /2 is because central_peak has x and y coords
...
for wav=0,dim3-1 do begin
   ...
   for i=0,n_ps-1 do begin
      ft_frame = ft_arr[*,*,i,wav]
      ft_frame[w] = 0.0
      autocor_noise[*,*,wav]+= double(fft(modsq(fft(ft_frame,-1)),1))
   endfor
endfor
...
for wav=0,dim3-1 do for j = 0,n_baselines - 1 do for k = 0,n_baselines-1 do $ 
 v2_cov[j,k,wav] = total(v2diff[*,j,wav] * v2diff[*,k,wav])/(n_blocks - 1.0)/double(n_blocks)
...
for wav=0,dim3-1 do temp_v2_cov[*,wav]=v2_cov[x,x,dblarr(n_baselines)+wav]
...
if keyword_set(average_v2_not_power) then begin


   for wav=0,dim3-1 do begin
      for j=0,n_ps -1 do begin
         bs_all[j,*,wav] = complex( bs_arr[j,*,wav]/( fluxes[j,wav]^3 ) * n_holes^3 )
         v2_all[j,*,wav] = v2_arr[j,*,wav] / ( fluxes[j,wav]^2 ) * n_holes^2
         cvis_all[j,*,wav] = cvis_arr[j,*,wav] / ( fluxes[j,wav] ) * n_holes
      endfor
   endfor

   ;; Unfortunately we have to do these again...
   v2=mean(v2_all,dim=1)
   bs = mean(bs_all,dim=1)

   ;; These variables cant be done this way without a major rewrite...
   for wav=0,dim3-1 do begin
      ...
      avar[*,wav] = avar[*,wav]/mean(fluxes[*,wav]^4)*n_holes^4 > smallfloat
      err_avar[*,wav] = err_avar[*,wav]/mean(fluxes[*,wav]^4)*n_holes^4 > smallfloat
      fluxes[*,wav] = fluxes[*,wav]/10000.0 ;Prevent floating overflows...
      bs_v2_cov[*,*,wav] = real_part(bs_v2_cov[*,*,wav]/mean(fluxes[*,wav]^5)*n_holes^5)/10000.0^5
      bs_cov[*,*,wav] = double(bs_cov[*,*,wav]/mean(fluxes[*,wav]^6d)*n_holes^6d)/10000.0^6d ;; this won't work properly if the flux changes between frames...
      bs_var[*,*,wav] = (double(bs_var[*,*,wav]/mean(fluxes[*,wav]^6d)*n_holes^6d)/10000.0^6d) > smallfloat
   endfor

   ;; We need to fix the uncertainties so they make sense
   for wav=0,dim3-1 do begin
      v2_cov[ix,ix,wav+intarr(n_baselines)]=variance(v2_all[*,*,wav],dim=1)
   endfor
   ; bs_var
   ...
   for wav=0,dim3-1 do begin
      bs_all[*,*,wav] = complex( bs_arr[*,*,wav]/mean( fluxes[*,wav]^3 ) * n_holes^3 )
      v2_all[*,*,wav] = v2_arr[*,*,wav] / mean( fluxes[*,wav]^2 ) * n_holes^2
      cvis_all[*,*,wav] = cvis_arr[*,*,wav] / mean( fluxes[*,wav] ) * n_holes
   endfor
...
;2) fit to the phase slopes using weighted linear regression.
 ...
 err2_bias=err2
 for wav=0,dim3-1 do begin ;; loop over wavs
    for j = 0,n_ps-1 do begin ;; loop over frames
       hole_phs[0,j,*,wav]=$
