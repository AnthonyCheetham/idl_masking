...
pro bispect_edit, ft_arr, mf_pvct, mf_gvct, mf_ix, mf_rmat, mf_imat, v2, v2_cov, bs,bs_var, bs_cov, bs_v2_cov, $
...
bs_all=complexarr(n_blocks,n_bispect) ;; this used to say complex...
v2_all=dblarr(n_blocks,n_baselines);; this used to say double...
cvis_all=complexarr(n_blocks,n_baselines)
for fr=0,n_ps-1 do begin ;;ACC added this loop to test flux
   bs_all[fr,*] = complex( bs_arr[fr,*]/fluxes[fr]^3 * n_holes^3 )
   v2_all[fr,*] = v2_arr[fr,*] / fluxes[fr]^2 * n_holes^2
   cvis_all[fr,*] = cvis_arr[fr,*] / fluxes[fr] * n_holes
endfor
...
for j = 0,n_baselines - 1 do for k = 0,n_baselines-1 do $ 
  v2_cov[j,k] = total(v2diff[*,j] * v2diff[*,k])/(n_blocks - 1d)/float(n_blocks)
...
;bs_v2_cov = real_part(bs_v2_cov/mean(fluxes^5)*n_holes^5)/10000d^5
;bs_cov = double(bs_cov/mean(fluxes^6)*n_holes^6)/10000d^6
;bs_var = (double(bs_var/mean(fluxes^6)*n_holes^6)/10000d^6) > smallfloat
