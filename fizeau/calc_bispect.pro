...
;naive_cp_uncerts: calculate the closure phase uncertainties as their standard 
;                  deviation, rather than the complicated way that bispect does it.
pro calc_bispect, cubeinfo_file,  root_dir=root_dir, ddir = ddir,  reset = reset,  plotall =plotall,  nosave = nosave, $
                  ...
                  subtract_bs_bias = subtract_bs_bias, window_mult=window_mult, special=special, cav=cav, rebin=rebin,disp_windowing=disp_windowing,naive_cp_uncerts=naive_cp_uncerts
...
if (keyword_set(window_size) eq 0) then begin
    window_size = 0.8d*imsize ;1.3 suppresses, but still includes thr first airy ring.
...
for i=0, n_runs-1 do begin
    ...
    window =  dblarr(dimx, dimy)
    if (window_type[0] eq 1) then $
      window[0:dim_min-1, 0:dim_min-1] = exp(-(dist(dim_min)/window_size*0.91244d)^4) $ 
    else if (window_type[0] eq 0) then $
      window[*] =  1.0d
    if(median_sub ne 0) then begin
        if (window_size gt dimx/2) then stop
        if (dimx ne dimy) then stop ;Following line will not work
        w =  where(shift(dist(dimx), dimx/2.d0, dimx/2.d0) gt 1.2d*window_size)
    ...
    for k = 0, n_frames-1 do begin
        ...
        if keyword_set(disp_windowing) then begin ;;ACC: useful for checking windowing (e.g. make sure a wide companion is suppressed)
           if k eq 0 then begin
              !p.multi=[0,2,1]
              image_cont,shift(shift(cube[*,*,k], -dimx/2.d0,  -dimy/2.d0)*window,128,128),/n,/a,tit='Windowed'
              image_cont,shift(shift(cube[*,*,k], -dimx/2.d0,  -dimy/2.d0),128,128),/n,/a,tit='No Window'
              wait,0.75
           endif
        endif
        if (showps eq 1) then begin
           temp =  modsq(ft_cube[0, 0, k])/10d
           image_cont, shift(modsq(ft_cube[*,*,k]) < temp,dimx/2d,dimy/2d)^0.5d, /noc
           wait,  0.2
        endif
     endfor
    if (olog.dk_fname[i] ne '') then for k =  0, n_dframes -1 do $
      ft_dcube[*,*,k] = fft(shift(dcube[*,*,k], -dimx/2d,  -dimy/2d)*window, 1)
    ...
    if (olog.dk_fname[i] ne '') then begin
        ...
        dps[0, 0] =  0d        ;
        ft_cube[0, 0, *] -= total(ft_dcube[0, 0, *])/double(n_dframes) ;As this is used for bias subtraction in bispect
    endif else dps = dblarr(dimx,dimy)
    ...
    m = max(v2)
    ploterr, sqrt(u^2+v^2), v2, sqrt(v2_cov[in,in]), psym=5, yr = [0d,min([m,1d])],$
    ...
    if (keyword_set(nosave) eq 0) then begin
       if (i eq 0) then bs_names =  'bs'+olog.cube_fname[i,1]+ special+'.idlvar' $
        ...
        if keyword_set(naive_cp_uncerts) then begin
           cp_all=atan(bs_all,/phase)
           cp_sig=stddev(cp_all,dim=2)
        endif
        ;stop
...
; if (plotall eq 0) then window,  1,  xsize = 700,  ysize = 550, ypos=50
    ...
    mask =  ((mf_tot < 0.01d)*100d) < 0.995d

; image_cont, shift(alog((ps > max(ps)*1e-7)*(1-mask)),dimx/2,dimy/2), /nocont, /asp
                                ;loadct,  0
    image_cont, shift(alog((ps > max(ps)*1d-7)*(1-mask)),dimx/2,dimy/2), /nocont, /asp 
 ;   !p.multi = 0
