...
pro raw2cube_dither_nirc2,speck,datadir=datadir,  		$
  ...
  destripe = destripe, medsub_ssky=medsub_ssky, manclick=manclick,root_dir=root_dir
...
;;ACC NOTE: This is not my code, but ive seen it remove part of the speckle pattern in some data! (Caused by shifting not actually keeping star in one place, due to crappy ra and dec from the headers (I've seen it 55arcsec off). Cant we just do this later, when we actually know where the star has shifted to?
...
if (skies[0] eq -1) then $   
 ...
 ssky=0
 darks = dcube
endif else if (skies[0] eq -3) then begin
   ;;ACC: I added this to help with data that had no dithering and no sky frames.
   ;;This just sets the darks to 0 and the sky frames to zero
   ssky=0
   szd=size(dcube)
   darks=randomn(seed,szd[1],szd[2],szd[3])*median(dcube)
endif else begin
 ; %%%%%  Begin loop over data frames 
...
;  im = nirc2warp(im0,hd=head)
  im=nirc2dewarp(im0,hd=head) ;;new distortion solution
 endif
...
;     if(dimx gt 256) then tvscl,rebin(im,256,256) else tvscl,im
     ...
     al_legend,['File n'+string(speck[i],format="(I4.4)")+".fits"]
